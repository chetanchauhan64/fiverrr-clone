...
export const getGigData = async (req, res, next) => {
  try {
    if (req.params.gigId) {
      ...
      const totalRatingSum = userWithGigs?.gigs?.reduce(
        (acc, gig) =>
...
const createSearchQuery = (searchTerm, category) => {
  ...
  if (searchTerm || category) {
    ...
    if (category) {
      // Robust category check: handles mismatch between Graphics & Design vs Graphic Design
      if (category.toLowerCase().includes("graphic")) {
        query.where.OR.push({
          category: { contains: "Graphic", mode: "insensitive" },
        });
      } else {
        query.where.OR.push({
